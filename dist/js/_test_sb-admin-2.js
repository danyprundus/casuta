...
dialog_client_close = $( "#dialog-form-client-close" ).dialog({
        autoOpen: false,
        height: 400,
        width: 350,
        modal: true,
        buttons: {
            "Inchide Client": closeClientRFID,
            Cancel: function() {
                dialog.dialog( "close" );
            }
        },
...
$( "#close-client" ).button().on( "click", function() {
            dialog_client_close.dialog( "open" );
...
function addClientRFID(){
    $(".clienti .nume").val($("#rfidcode_client").val());
    $(".clienti #adaug").click();
